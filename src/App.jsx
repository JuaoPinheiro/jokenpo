...
const messages = {
  ...
  computerWin:{
    title: 'Que Pena!',
    message: 'Não foi dessa vez, tente novamente aposto que você consegue!'
  },
  ...
  return (
    <C.Container>
      <C.Flex direction='column'>
        ...
        >JO KEN PÔ</C.Typography>
        <Input placeholder='Digite seu nome'
          onChange={(value) => handleUserName(value)}
        />
        <Button onClick={startGame} startGame>{playGame ? "Parar" : "Iniciar"}</Button>
        <Score
          userName={userName}
        ...
        <C.Flex justify='space-around'>
          <C.Typography size='35px'>{userAction}</C.Typography>
          <C.Typography size='35px'>{computerAction}</C.Typography>
        </C.Flex>

        <C.Flex direction='column' gap='0px'>
          <C.Typography fontWeight='450'>{textGame}</C.Typography>
